...
add_ext_M5_categ_feat = function(series_list) {
  #category of the product
  cat_ids = sapply( series_list, function (ll) {
    strsplit(ll$id, "_")[[1]][1]
  })
  cat_ids = keras::to_categorical(as.numeric(as.factor(cat_ids))-1)

  store_ids = sapply( series_list, function (ll) {
    paste( strsplit(ll$id, "_")[[1]][4], strsplit(ll$id, "_")[[1]][5], sep="")
  })
  store_ids = keras::to_categorical(as.numeric(as.factor(store_ids))-1)

  for (i in 1:length(series_list)) {
    series_list[[i]]$ext_feat = c(cat_ids[i,], store_ids[i,])
  }
  series_list
}
...
deep_model = function(X, y, lag, weights=NULL) {
  dnm = deepnet_ar_model(X, y, NULL, NULL, 150, 42, weights = weights)
  dnm$lag = lag
  dnm
}
...
analyze_glob_model = function(my_series, fit_model_fun, model_name="custom_AR", MASE_norm=TRUE, lag_range=NULL, do_weights = FALSE,
 ...
 #                       series_list)
  ...
  if(ret_series) {
    attr(err_results, "forec_series") <- series_list
  }
