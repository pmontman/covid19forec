...
round_absolute_loss = function(y_true, y_pred) {
 #   k_mean( k_maximum( k_abs(y_true - y_pred), 0.15 ), keepdims = TRUE)
    loss = k_maximum( k_abs(y_true - y_pred), 0.2 )
    true = y_true*0
    loss_mean_absolute_error(true, loss)
}

round_square_loss = function(y_true, y_pred) {
  k_mean( k_maximum( k_square(y_true - y_pred), 0.015 ))
}
...
deepnet_ar_model <- function(X_train, y_train, X_val, y_val, epochs=350, batch=1, patience=ceiling(epochs/10),
  ...
  deep_model <- keras_model_sequential() %>%
   ...
   # layer_batch_normalization() %>%
    ...
    compile(
      loss =  'mean_absolute_error',#'mean_absolute_error', #abs_var_penal_loss, #
